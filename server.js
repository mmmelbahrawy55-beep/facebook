...
app.post('/api/login', async (req, res) => {
  try {
    ...
    if (!chromePath) {
      return res.json({ success: false, message: 'لم يتم العثور على Chrome' });
    }
...
app.post('/api/post', async (req, res) => {
  ...
  try {
    const searchTerms = [`${governorate} وظائف`, `${governorate} توظيف`];
    let allGroups = [];
    ...
    console.log(`اكتمل: ${successCount} نجح, ${failCount} فشل`);
  } catch (error) {
